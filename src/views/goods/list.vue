          ...
          <template #default="{ row }">
            <div class="flex">
              ...
              <div class="flex-1">
                <p>{{ row.title }}</p>
                <div>
                  <span class="text-rose-500">&yen;{{ row.min_price }}</span>
          ...
          <template #default="{ row }">
            <div class="flex flex-col" v-if="row.ischeck == 0">
              ...
              <el-button class="mt-2 !ml-0" type="danger" size="small" plain
                @click="handleCheckGoods(row, 2)">审核拒绝</el-button>
            </div>
            <span v-else>{{ row.ischeck == 1 ? '通过' : '拒绝' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="总库存" width="100" prop="stock" align="center" />
          ...
          <template #default="scope">
            <div v-if="searchForm.tab != 'delete'">
              ...
              <el-button class="px-1"
                ...
                size="small" text :loading="scope.row.skusLoading" @click="handleSetGoodsSkus(scope.row)">商品规格</el-button>
